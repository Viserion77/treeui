...
danger | info`), orthogonal to `variant` and using the same closed vocabulary
  ...
  - **`TTagInput` exposes its pending draft** — `v-model:draft` plus a `hasDraft`
    ...
    when the draft is the only change: a click on a _disabled_ Save fires no
...
- 1a9d136: Fix a regression from 0.28.0: `TFormField` emitted `<label for>`
...
- 4199e28: Fix `TMenu` never opening from its trigger.

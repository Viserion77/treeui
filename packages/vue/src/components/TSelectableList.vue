...
export interface TSelectableListItem {
  label: string;
  value: string;
  description?: string;
  meta?: string;
...
const baseId = createId('t-selectable-list');
...
const selectItem = (item: TSelectableListItem, focus = false) => {
  ...
  >
    <template v-if="items.length > 0">
      ...
      >
        ...
        >
          <span
            class="t-selectable-list__indicator"
            aria-hidden="true"
          />

          <span class="t-selectable-list__copy">
            <span class="t-selectable-list__label">{{ item.label }}</span>
            <span
              v-if="item.description"
              class="t-selectable-list__description"
            >
              {{ item.description }}
            </span>
          </span>
    ...
    <div
      v-else
      class="t-selectable-list__empty"
    >
      <slot name="empty">
        {{ emptyText }}
      </slot>
